...
class MentorRequests extends React.Component {
	...
	constructor () {
  	...
  	componentDidMount () {
	    ...
	    axios.get(`http://mentors/api/mentor/${ReactSession.get("mem_id")}/requests/all`).then(response => {
	      ...
	      // test
	      for(let i = 0; i < this.state.requests.pending_requests.length; i++){
	      	console.log(this.state.requests.pending_requests[i].conn_id);
	      }
	...
	render() {
    	...
    	let pending_requests = this.state.requests.pending_requests
    	let accepted_requests = this.state.requests.accepted_requests
    	let unsuccessful_requests = this.state.requests.unsuccessful_requests
